async function setMap(rent_data, region, view) {
  ...
  for (let i = 0; i < station_data.length; i++) {
    if (region == "전체" || station_data[i]["자치구"] == region) {
      ...
      let temp_diff = Math.abs(
        station_data[i]["returned"] - station_data[i]["rented"]
      );
  ...
  gangnam_bike_stations.forEach(function (d) {
    ...
    if (view == 1) {
      ...
      if (temp_val >= 0) {
        circle_color = pickHex(
          slider_color[2],
          slider_color[1],
          Math.abs(temp_val) / max_diff
        );
      } else {
        circle_color = pickHex(
          slider_color[0],
      ...
      circle_radius =
        ((Math.abs(temp_val) - min_diff) /
          Math.max(2, (max_diff - min_diff) / binning) +
          1) *
        radius_interval;
    } else if (view == 0) {
      // all
      ...
      circle_color = pickHex(
        slider_color_all[0],
        slider_color_all[1],
        Math.abs(temp_val) / total_max
      );
      circle_radius =
        ((temp_val - total_min) /
          Math.max(2, (total_max - total_min) / binning) +
          1) *
        radius_interval;
    }
...
function updateBySlider(mode) {
  ...
  mymap.eachLayer(function (layer) {
    ...
    removed_layer.forEach(function (layer) {
      if (
        layer.options.value >= min_thresh &&
        layer.options.value <= max_thresh
      ) {
        layer.addTo(mymap);
...
async function main() {
  ...
  mymap = L.map("map").setView([37.56, 127.0], 11);
