        ...
        if(region == '전체' || station_data[i]['자치구'] == region){
            ...
            if(rent_data != undefined){
                if(rent_data[station_data[i]['대여소번호']] != undefined){
                    station_data[i]['rented'] = rent_data[station_data[i]['대여소번호']].rented;
                    station_data[i]['returned'] = rent_data[station_data[i]['대여소번호']].returned;
                }
            }
...
let station_data;
let mymap;
let regions = ['전체', '종로구', '중구', '용산구', '성동구', '광진구', '동대문구', '중랑구', '성북구', '강북구', '도봉구', '노원구', '은평구', '서대문구', '마포구', '양천구', '강서구', '구로구', '금천구', '영등포구', '동작구', '관악구', '서초구', '강남구', '송파구', '강동구'];
...
function updateMap(option){
    ...
    mymap.eachLayer(function(layer) {
        if(layer.options.className == 'value')
